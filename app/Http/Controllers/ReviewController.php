...
class ReviewController extends Controller
{
    ...
    public function printReportBulanan(Request $request)
    {
        ...
        $result = ResultReviewPtsp::groupBy('officer_id')->select('officer_id', DB::raw('sum(result) as sum_res'), DB::raw('count(result) as count_res'), DB::raw('((sum(result)/count(result))*100)/5 as avg'), DB::raw('sum(result_komunikasi) as sum_res_komunikasi'), DB::raw('count(result_komunikasi) as count_res_komunikasi'), DB::raw('((sum(result_komunikasi)/count(result_komunikasi))*100)/5 as avg_komunikasi'), DB::raw('sum(result_kompetensi) as sum_res_kompetensi'), DB::raw('count(result_kompetensi) as count_res_kompetensi'), DB::raw('((sum(result_kompetensi)/count(result_kompetensi))*100)/5 as avg_kompetensi'))->whereMonth('assessment_date', $request->month)->whereYear('assessment_date', $request->year)->get();
        $dataPrint = [];
        if (!$result->isEmpty()) {
            ...
            foreach ($result as $res) {
                $dataPrint[] = [
                    ...
                    'integritas' => number_format($res->avg, '2', ',', '.'),
                    'komunikasi' => number_format($res->avg_komunikasi, '2', ',', '.'),
                    'kompetensi' => number_format($res->avg_kompetensi, '2', ',', '.'),
                ];
            }
        } else {
            $dataPrint[] = [
        ...
        if (!$result->isEmpty()) {
            ...
            foreach ($result as $res) {
                $dataPrint[] = [
                    ...
                    'integritas' => $res->result,
                    'komunikasi' => $res->result_komunikasi,
                    'kompetensi' => $res->result_kompetensi,
    ...
    public function printReportBulananSatpam(Request $request)
    {
        ...
        $result = ResultReviewSatpam::groupBy('officer_id')->select('officer_id', DB::raw('sum(result) as sum_res'), DB::raw('count(result) as count_res'), DB::raw('((sum(result)/count(result))*100)/5 as avg'), DB::raw('sum(result_komunikasi) as sum_res_komunikasi'), DB::raw('count(result_komunikasi) as count_res_komunikasi'), DB::raw('((sum(result_komunikasi)/count(result_komunikasi))*100)/5 as avg_komunikasi'), DB::raw('sum(result_kompetensi) as sum_res_kompetensi'), DB::raw('count(result_kompetensi) as count_res_kompetensi'), DB::raw('((sum(result_kompetensi)/count(result_kompetensi))*100)/5 as avg_kompetensi'))->whereMonth('assessment_date', $request->month)->whereYear('assessment_date', $request->year)->get();
        $dataPrint = [];
        if (!$result->isEmpty()) {
