...
<div class="col bg-white shadow p-5">
    ...
    <h1>Petugas Satpam</h1>
    <div class="row mb-2">
        <div class="col-md-6 d-flex">
            <button class="btn btn-primary add-petugas-satpam">Tambah petugas</button>
...
<script type="text/javascript">
    $(function () {
        ...
        var table = $('.table').DataTable({
                ...
                serverSide: true,
                responsive:true,
                ajax: "{{ url('review/get_petugas_satpam') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
    ...
    $('.add-petugas-satpam').click(function(e){
        e.preventDefault();
        
        $.ajax({
            type: "get",
            url: "{{ url('review/modal_tambah_petugas_satpam') }}",
           
            dataType: "json",
            success: function (response) {
                console.log(response.modal)
                $('.modal_container').html(response.modal);
                var modalPetugasPtsp = new bootstrap.Modal(document.querySelector('.modal_container .tambah_petugas_satpam'));
    ...
    $('table tbody').on('click','.update-btn', function(e){
        ...
        $.ajax({
            type: "post",
            url: "{{ url('review/modal_edit_satpam') }}",
            data: {
                id_pegawai
            },
            dataType: "json",
